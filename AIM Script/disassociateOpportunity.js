function getRelatedAppointments(executionContext)
{
  ...
  var pfDisbursementName = formContext.getAttribute("mphhi_pfdisbursementname").getValue(); // get pf disbursement name
  ...
  // remove related appointment on change
  ...
  retrieveMultipleAppointmentRecords(executionContext, pfDisbursementName);

}

function retrieveMultipleAppointmentRecords(executionContext, pfDisbursementName) {
  var formContext = executionContext.getFormContext();
  var pfID= formContext.data.entity.getId(); // PF Disburment GUID
  var pfIDSlice = pfID.slice(1,-1); // remove brackets from GUID
  var disbursementStatus = formContext.getAttribute("statuscode").getValue(); // get Status Reason value
  ...
  var totalNonVatAmount = 0;
  var totalDiscountAmount = 0;
  var totalGrossAmount = 0;

  // and _msemr_actorlocation_value eq 123 -- pending
  Xrm.WebApi.online.retrieveMultipleRecords("msemr_appointmentemr", "?$select=mphhi_appointmentguid,mphhi_appointmentnumber,mphhi_datedevfield,mphhi_discountamount,_mphhi_doctor_value,mphhi_grossamount,mphhi_netamount,mphhi_netamountnonvatable,mphhi_netamountvatable,mphhi_startmonth,_msemr_actorlocation_value&$filter=mphhi_disburseddate eq '"+pfDisbursementName+"'").then(
    function success(results) {
        for (var i = 0; i < results.entities.length; i++) {
          var mphhi_appointmentguid = results.entities[i]["mphhi_appointmentguid"];
          ...
          var mphhi_netamountnonvatable = results.entities[i]["mphhi_netamountnonvatable"];
          var mphhi_netamountnonvatable_formatted = results.entities[i]["mphhi_netamountnonvatable@OData.Community.Display.V1.FormattedValue"];
          var mphhi_netamountvatable = results.entities[i]["mphhi_netamountvatable"];
          var mphhi_netamountvatable_formatted = results.entities[i]["mphhi_netamountvatable@OData.Community.Display.V1.FormattedValue"];
          ...
          totalNonVatAmount = totalNonVatAmount + mphhi_netamountnonvatable;
          totalVatAmount = totalVatAmount + mphhi_netamountvatable;
        ...
        Xrm.Page.getAttribute("mphhi_netamountvat").setValue(totalNonVatAmount);
        Xrm.Page.getAttribute("mphhi_netamountnonvatable").setValue(totalVatAmount);
...
function removePFDisbursementValueInAppointment(executionContext,mphhi_appointmentguid) {
    ...
    var req = new XMLHttpRequest();
    req.open("DELETE", Xrm.Page.context.getClientUrl() + "/api/data/v9.1/opportunities("+mphhi_appointmentguid+")/ndph_opportunity/$ref", true);
    req.setRequestHeader("Accept", "application/json");
    req.setRequestHeader("Content-Type", "application/json; charset=utf-8");
    req.setRequestHeader("OData-MaxVersion", "4.0");
    req.setRequestHeader("OData-Version", "4.0");
