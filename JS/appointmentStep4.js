$(document).ready(function () {
    ...
    // ****************************************************************
    // disable submit button if not agree
    $("#mphhi_agreetotelemedconditions").change(function () {
    ...
    */
    ...
    var availableDates = [];    // Declare outside for use outside
    function showPreferredDateAndTimeslot() {
        ...
        if (preferredDoctor.val()) {
            ...
            availableDates = queryAvailableTimeslotDates(preferredDoctor.val(), appointmentType.val());
            // Subtract available dates from all possible dates to get blacklisted dates
            var blacklistedDates = possibleDates.filter(e => !availableDates.includes(e));      // possibleDates array defined near top
            // disabledDates method does not seem to accept ISO date format; need to convert to a date object
    ...
    preferredDate.closest("div.control").on("dp.change", 
        function() {
            ...
            if (preferredDate.val() && availableDates.length > 0) {     // availableDates should already be populated by the time Preferred Date is visible; this is just a failsafe
                // Search for corresponding date
                var currentSelectedDate = preferredDate.val().split("T")[0];
                var currentTimeslotDate = availableDates.find(date => date.mphhi_date.startsWith(currentSelectedDate));
